...
; pmtest8.asm
; author: Liu, Linhong
; compile: nasm pmtest9.asm -o pmtest9.com
; ===================================================================
...
[BITS	32]
LABEL_IDT:
; GATE			Target Selector		   offset	      Dcount	  proper
%rep 32
...
Init8259A:
	mov al, 011h
	out 020h, al	; icw1 for master 8259
	call io_delay

	out 0A0h, al    ; icw1 for slave 8259
	call io_delay

	mov al, 020h     ; icw2 for master, irq0 -> 0x20
	out 021h, al
	call io_delay

	mov al, 028h 	; icw2 for slave, irq8 -> 0x28
	out 0A1h, al
	call io_delay

	mov al, 004h	; irq2 for slave
	out 021h, al	; icw3 for master
	call io_delay

	mov al, 002h	; target irq2 of master
	out 0A1h, al	; icw3 for slave
	call io_delay

	mov al, 001h
	out 021h, al	; icw4 for master
	call io_delay

	out 0A1h, al	; icw4 for slave
	call io_delay

	mov al, 11111110b ; unmask irq0 the clock interrupt
	out 021h, al	; ocw1 for master
	call io_delay 
...
_SupriousHandler:
SpuriousHandler	equ _SupriousHandler - $$
	mov ah, 0Ch
	mov al, '!'
	mov [gs:((80 * 0 + 75) * 2)], ax
	jmp $
